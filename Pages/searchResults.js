...
export default class searchResults{
    ...
    async addToCart(){
      ...
      const titleOfBook=  await this.actions.getText('.product__title__main-part'); 
      ...
      console.log(`Authors of the book:${authorsOfTheBook.join(',')}`);
        ...
        console.log(`First author of the book:${selectedFirstAuthor}`);
         ...
         const selectedSecondAuthor= await this.actions.getFirstAuthor('.product__stand-alone-attribute--authors a span', 1);
        console.log(`First author of the book:${selectedSecondAuthor}`);

        //4. Print the rating
        const rating=await this.actions.getRating('.product__stars__wrapper','title');
        ...
        await this.actions.wait();
        const CarStep=await this.actions.getByRole('heading', 'Steg 1 - Din varukorg');
        console.log(`Step 1 in the cart:${CarStep}`);
